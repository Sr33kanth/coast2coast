...
// Load route polyline from cached GeoJSON
function getRoutePolyline(): LatLngExpression[] {
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const coords = (routeData as any).features[0].geometry.coordinates;
    return coords.map(([lng, lat]: [number, number]) => [lat, lng] as LatLngExpression);
  } catch {
    return [];
  }
}
...
const MapView: React.FC = () => {
  ...
  const mapCenter: [number, number] = [39.8283, -98.5795]; // Center of US
    ...
    const loadData = async () => {
      try {
        ...
        if (checkInsData.length > 0) {
          setCurrentLocation(checkInsData[0]);
        }
  ...
  return (
    <div className="h-[600px] w-full rounded-xl overflow-hidden shadow-lg" id="map-section">
      ...
      >
        ...
        {/* Photos */}
        {photos.filter(p => p.lat !== 0 && p.lng !== 0).map((photo) => (
          <Marker
            key={photo.id}
